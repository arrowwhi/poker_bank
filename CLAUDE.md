...
Дельта `bank_delta_rub = bank(game_id)` перераспределяется так, чтобы бо́льшая из двух сумм — долг должников (X) или выигрыш кредиторов (Y) — пропорционально УМЕНЬШИЛАСЬ до размера меньшей; меньшая сторона не меняется:

- `delta > 0` (выдано фишек больше, чем возвращено): кредиторы получают выигрыш полностью, долг должников пропорционально уменьшается, пока X не сравняется с Y.
- `delta < 0` (возвращено фишек больше, чем выдано): должники платят долг полностью, выигрыш кредиторов пропорционально уменьшается, пока Y не сравняется с X.

Правило: никто не платит и не получает больше расчётного значения — корректировка только уменьшает, никогда не увеличивает. После этого запускается алгоритм переводов. Дельта сохраняется в `games.bank_delta_rub` для прозрачности.
...
| Что с расхождением банка | `/endgame_force` пропорционально уменьшает бо́льшую из двух сумм (долг должников или выигрыш кредиторов) до размера меньшей; меньшая сторона не меняется — никто не платит/не получает больше расчётного |
